 ...
 * Run with: node scripts/generateAssets.mjs
 */
...
function generateTileset() {
  ...
  fs.writeFileSync(path.join(rootDir, 'public/assets/tilesets/tileset.png'), buffer);
  console.log('Generated tileset.png');
}

function drawGrass(ctx, tileX, tileY, color) {
...
function drawGrassWithFlower(ctx, tileX, tileY) {
...
function drawTree(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawRock(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawPath(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawWater(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawSand(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawHouseTop(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawHouseBottom(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawDoor(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawWindow(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawSign(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawBush(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function drawFence(ctx, tileX, tileY) {
  const x = tileX * TILE_SIZE;
  const y = tileY * TILE_SIZE;
...
function generatePlayerSprite() {
  ...
  const buffer = canvas.toBuffer('image/png');
  fs.writeFileSync(path.join(rootDir, 'public/assets/characters/player.png'), buffer);
  console.log('Generated player.png');
}

function drawPlayerFrame(ctx, frameX, frameY, colors, animFrame) {
...
function generateNPCSprite() {
  ...
  const buffer = canvas.toBuffer('image/png');
  fs.writeFileSync(path.join(rootDir, 'public/assets/characters/npc.png'), buffer);
  console.log('Generated npc.png');
}

function drawNPCFrame(ctx, frameX, frameY, colors, direction) {
  const x = frameX * TILE_SIZE;
  const y = frameY * TILE_SIZE;
